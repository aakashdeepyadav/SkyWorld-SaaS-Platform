...
const healthLabel = { up: 'Online', down: 'Offline', degraded: 'Degraded', connected: 'Connected', disconnected: 'Disconnected', error: 'Error' };
const healthColor = { up: 'text-green-500', down: 'text-red-500', degraded: 'text-amber-500', connected: 'text-green-500', disconnected: 'text-slate-400', error: 'text-red-500' };
const healthDot   = { up: 'bg-green-500', down: 'bg-red-500', degraded: 'bg-amber-500', connected: 'bg-green-500', disconnected: 'bg-slate-400', error: 'bg-red-500' };

const formatTime12 = (t) => { if (!t) return ''; const [h, m] = t.split(':').map(Number); return `${h % 12 || 12}:${String(m).padStart(2, '0')} ${h >= 12 ? 'PM' : 'AM'}`; };
...
const StatCard = ({ icon: Icon, label, value, sub, accent = 'sky' }) => {
  ...
  const iconAccent = {
    sky: 'text-sky-500', green: 'text-green-500', amber: 'text-amber-500',
    red: 'text-red-500', violet: 'text-violet-500', slate: 'text-slate-500',
  };

  return (
    <div className={`bg-gradient-to-br ${accents[accent]} border rounded-2xl p-5 flex items-start gap-4 transition-all hover:shadow-md`}>
...
const HealthRow = ({ icon: Icon, label, status }) => (
  <div className="flex items-center justify-between py-2.5">
    ...
    <div className="flex items-center gap-2">
      <span className={`inline-block h-2 w-2 rounded-full ${healthDot[status] || 'bg-slate-400'}`} />
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-xs">
        ...
        <span className="text-slate-500 dark:text-slate-400">{sent} / {limit} &middot; <span className="font-semibold">{remaining} left</span></span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            {tab === 'live' ? `Live insights for ${d.date || 'today'}` : 'Historical analytics from daily snapshots'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {/* Tabs */}
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                <StatCard icon={CalendarDaysIcon}            label="Meetings"       value={m.total || 0}                   sub={`${m.confirmed || 0} confirmed`}   accent="sky" />
                <StatCard icon={CreditCardIcon}              label="Payments"       value={p.total || 0}                   sub={`₹${(p.revenue || 0).toLocaleString('en-IN')} revenue`}  accent="green" />
                <StatCard icon={EnvelopeIcon}                label="Emails Sent"    value={(eb.sent || 0) + (er.sent || 0)} sub={`Brevo ${eb.sent || 0} · Resend ${er.sent || 0}`} accent="violet" />
                <StatCard icon={ClipboardDocumentListIcon}   label="Requests"       value={(d.serviceRequests?.total || 0) + (d.customRequests?.total || 0)} sub={`${d.serviceRequests?.pending || 0} pending`} accent="amber" />
                <StatCard icon={UserGroupIcon}               label="Users"          value={d.users?.total || 0}            sub={`+${d.users?.newToday || 0} today`} accent="slate" />
                <StatCard icon={ArrowTrendingUpIcon}         label="Projects"       value={d.projects?.total || 0}         sub={`${d.projects?.active || 0} active`} accent="sky" />
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="lg:col-span-1 bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-5">
                  ...
                  {h.lastCheckedAt && (
                    <p className="text-[11px] text-slate-400 mt-3">Last checked {new Date(h.lastCheckedAt).toLocaleTimeString('en-IN')}</p>
                  )}
                </div>
                ...
                <div className="lg:col-span-2 bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-5">
                  ...
                  <div className="space-y-5">
                    ...
                    <QuotaBar label="Resend (Meeting Emails)" sent={er.sent || 0} limit={er.limit || 100} remaining={er.remaining ?? 100} color="violet" />
                  </div>
                  <p className="text-[11px] text-slate-400 mt-4">Quotas reset at midnight. Data based on today's usage.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-5">
                  ...
                  {paymentPieData.length > 0 ? (
                    <ResponsiveContainer width="100%" height={220}>
                      <PieChart>
                        <Pie data={paymentPieData} cx="50%" cy="50%" outerRadius={80} innerRadius={45} dataKey="value" paddingAngle={3} label={({ name, value }) => `${name} (${value})`}>
              ...
              {/* ── Meetings Table ── */}
              <div className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-5">
                <SectionHeader icon={VideoCameraIcon} title={`Today's Meetings (${m.total || 0})`} />
                {m.list && m.list.length > 0 ? (
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                      <thead>
                        <tr className="border-b border-slate-200 dark:border-slate-700">
                          <th className="py-2.5 px-3 font-semibold text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wide">Client</th>
                          <th className="py-2.5 px-3 font-semibold text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wide">Email</th>
                          <th className="py-2.5 px-3 font-semibold text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wide">Time</th>
                          <th className="py-2.5 px-3 font-semibold text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wide">Meet Link</th>
                          <th className="py-2.5 px-3 font-semibold text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wide">Status</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                        {m.list.map((mtg, i) => (
                          <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                            <td className="py-2.5 px-3 font-medium text-slate-800 dark:text-slate-200">{mtg.clientName}</td>
                            <td className="py-2.5 px-3 text-slate-500 dark:text-slate-400">{mtg.clientEmail}</td>
                            <td className="py-2.5 px-3 text-slate-700 dark:text-slate-300 whitespace-nowrap">{formatTime12(mtg.startTime)} – {formatTime12(mtg.endTime)}</td>
                            <td className="py-2.5 px-3">
                              {mtg.meetLink ? (
                                <a href={mtg.meetLink} target="_blank" rel="noopener noreferrer" className="text-sky-500 hover:underline text-xs font-medium">Join</a>
                              ) : <span className="text-slate-400 text-xs">—</span>}
                            </td>
                            <td className="py-2.5 px-3">
                              <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${meetingStatusStyle[mtg.status] || 'bg-slate-100 text-slate-500'}`}>{mtg.status}</span>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                  ...
                  <p className="text-sm text-slate-400 py-8 text-center">No meetings scheduled for today</p>
                )}
              </div>
            </>
          )}
        ...
        <div className="space-y-4">
          ...
          {historyLoading ? (
                ...
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="bg-slate-50 dark:bg-slate-700/40 border-b border-slate-200 dark:border-slate-700">
                      {['Date', 'Meetings', 'Payments', 'Revenue', 'Brevo', 'Resend', 'Backend', 'Frontend', 'New Users'].map((col) => (
                        <th key={col} className="py-3 px-4 font-semibold text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400 whitespace-nowrap">{col}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                    {historyData.map((row, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                        <td className="py-2.5 px-4 font-medium text-slate-800 dark:text-slate-200 whitespace-nowrap">{row.Date || row.date}</td>
                        <td className="py-2.5 px-4 text-slate-700 dark:text-slate-300">{row['Meetings Total'] ?? '—'}</td>
                        <td className="py-2.5 px-4 text-slate-700 dark:text-slate-300">{row['Payments Total'] ?? '—'}</td>
                        <td className="py-2.5 px-4 font-semibold text-green-600 dark:text-green-400">₹{Number(row['Revenue (₹)'] || 0).toLocaleString('en-IN')}</td>
                        <td className="py-2.5 px-4 text-slate-600 dark:text-slate-400">{row['Brevo Sent'] ?? '—'} / {row['Brevo Remaining'] ?? '—'}</td>
                        <td className="py-2.5 px-4 text-slate-600 dark:text-slate-400">{row['Resend Sent'] ?? '—'} / {row['Resend Remaining'] ?? '—'}</td>
                        <td className="py-2.5 px-4">
                          <span className={`inline-flex items-center gap-1 text-xs font-semibold ${row.Backend === 'up' ? 'text-green-500' : 'text-red-500'}`}>
                            {row.Backend === 'up' ? <CheckCircleIcon className="h-3.5 w-3.5" /> : <XCircleIcon className="h-3.5 w-3.5" />}
                            {row.Backend || '—'}
                          </span>
                        </td>
                        <td className="py-2.5 px-4">
                          <span className={`inline-flex items-center gap-1 text-xs font-semibold ${row.Frontend === 'up' ? 'text-green-500' : 'text-red-500'}`}>
                            {row.Frontend === 'up' ? <CheckCircleIcon className="h-3.5 w-3.5" /> : <XCircleIcon className="h-3.5 w-3.5" />}
                            {row.Frontend || '—'}
                          </span>
                        </td>
                        <td className="py-2.5 px-4 text-slate-700 dark:text-slate-300">{row['New Users'] ?? '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="text-center py-16 bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl">
              ...
              <p className="text-sm text-slate-400">No historical data yet. Snapshots are saved nightly at 11:59 PM IST.</p>
              <p className="text-xs text-slate-400 mt-1">Select a date range and click Load, or wait for the first nightly flush.</p>
            </div>
          )}
        </div>
      )}
