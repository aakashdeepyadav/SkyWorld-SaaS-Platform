...
const RequestDetail = () => {
        ...
        return (
            <div className="max-w-3xl mx-auto animate-fade-in">
                <div className="card animate-pulse space-y-4">
                    <div className="h-6 bg-gray-100 dark:bg-surface-700 rounded w-1/3" />
                    <div className="h-4 bg-gray-50 dark:bg-surface-600 rounded w-2/3" />
                    <div className="h-4 bg-gray-50 dark:bg-surface-600 rounded w-1/2" />
                </div>
            </div>
        );
    }
    ...
    if (error || !data) {
        return (
            <div className="max-w-3xl mx-auto text-center py-20">
                <p className="text-gray-500 dark:text-gray-400">Request not found or access denied.</p>
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
            <button onClick={() => navigate('/requests')} className="flex items-center text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors">
            ...
            <div className="card">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    <div className="p-3 bg-gray-50 dark:bg-surface-700 rounded-xl">
                        <p className="text-xs text-gray-400 dark:text-gray-500 mb-0.5">Service</p>
                        <p className="text-sm font-medium text-gray-900 dark:text-white">{data.serviceId?.name || 'N/A'}</p>
                    </div>
                    <div className="p-3 bg-gray-50 dark:bg-surface-700 rounded-xl">
                        <p className="text-xs text-gray-400 dark:text-gray-500 mb-0.5">Client</p>
                        <p className="text-sm font-medium text-gray-900 dark:text-white">{data.clientId?.name || 'N/A'}</p>
                    </div>
                    {data.estimatedPrice > 0 && (
                        <div className="p-3 bg-gray-50 dark:bg-surface-700 rounded-xl">
                            <p className="text-xs text-gray-400 dark:text-gray-500 mb-0.5">Estimated Price</p>
                            <p className="text-sm font-medium text-gray-900 dark:text-white">{formatINR(data.estimatedPrice)}</p>
                        </div>
                    )}
                    {data.assignedDeveloperId && (
                        <div className="p-3 bg-gray-50 dark:bg-surface-700 rounded-xl">
                            <p className="text-xs text-gray-400 dark:text-gray-500 mb-0.5">Assigned Developer</p>
                            <p className="text-sm font-medium text-gray-900 dark:text-white">{data.assignedDeveloperId.name}</p>
                        </div>
                    )}
                </div>
                ...
                <div className="mb-4">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed whitespace-pre-wrap">{data.description}</p>
                </div>

                {data.requirements && (
                    <div>
                        <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Requirements</h3>
                        <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed whitespace-pre-wrap">{data.requirements}</p>
                    </div>
                )}
            </div>
            ...
            {isAdmin && data.status === 'pending' && (
                <div className="card border-primary-200 bg-primary-50/30">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label htmlFor="devSelect" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Developer</label>
                            <select
                                id="devSelect"
                        ...
                        <div>
                            <label htmlFor="estPrice" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Estimated Price (INR)</label>
                            <input
                                id="estPrice"
                                type="number"
                                min="0"
