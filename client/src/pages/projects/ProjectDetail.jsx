...
const ProjectDetail = () => {
  ...
  const assignDeveloperMutation = useMutation(
    ...
    },
    {
      ...
      onError: (err) =>
        toast.error(err.response?.data?.message || err.message || 'Failed to assign developer'),
    }
  );
      ...
      const checkout = new window.Razorpay({
        ...
        handler: async (response) => {
          try {
            ...
            toast.success(
              isFinalPayment ? 'Final payment successful' : 'Advance payment successful'
            );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">
              {project.projectCode && (
                <span className="font-medium text-gray-500 mr-2">{project.projectCode}</span>
              )}
              Created {new Date(project.createdAt).toLocaleDateString()}
            </p>
          </div>
          <span className={`${getStatusBadge(project.status)} capitalize text-sm`}>
            {project.status}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          ...
          <div className="p-3 bg-gray-50 dark:bg-surface-700 rounded-xl">
            ...
            <p className="text-sm font-medium text-gray-900 dark:text-white">
              {project.budget ? formatINR(project.budget) : 'TBD'}
            </p>
          </div>
          <div className="p-3 bg-gray-50 dark:bg-surface-700 rounded-xl">
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-sm font-semibold text-gray-900 dark:text-white">
                {showMeetingPrompt
                  ? 'Payment received. Book your kickoff meeting.'
                  : 'Meeting & Contact'}
              </p>
              {showMeetingPrompt && <span className="badge-primary">Recommended</span>}
            </div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5">
              Schedule a meeting to discuss scope, timeline, and next steps. For urgent help,
        ...
        {isAdmin && (
          <div className="mb-6 p-4 bg-gray-50 dark:bg-surface-700 rounded-xl border border-gray-100 dark:border-surface-600">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              {project?.advancePaid && <span className="badge-primary">Payment Verified</span>}
            </div>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                disabled={
                  !project?.advancePaid || !selectedDeveloperId || assignDeveloperMutation.isLoading
                }
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {project.developerIds.map((dev) => (
                <div
                  key={dev._id || dev}
                  className="inline-flex items-center px-3 py-1.5 bg-gray-50 dark:bg-surface-700 rounded-lg text-sm"
                >
      ...
      <PaymentHistory projectId={id} totalPlanPrice={totalPlanPrice} />
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {project.status === 'review' && isAdmin && (
              <button
                ...
                className="btn-primary !bg-emerald-500 !text-sm !py-2"
              >
                Approve & Complete
              </button>
            )}
      ...
      <div className="card dark:bg-surface-800 dark:border-surface-700">
        ...
        >
          ...
          <svg
            className={`w-4 h-4 text-gray-400 transition-transform ${showMessages ? 'rotate-180' : ''}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
                  ...
                  return (
                    <div key={msg._id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                      ...
                      >
                        ...
                        <p
                          className={`text-xs mt-1 ${isMe ? 'text-primary-100' : 'text-gray-400'}`}
                        >
...
function PaymentHistory({ projectId, totalPlanPrice }) {
  const { data, isLoading } = useQuery(
    ['project-payments', projectId],
    async () => {
      const res = await api.get(`/payments?projectId=${projectId}&limit=20`);
      return res.data;
    },
    { enabled: !!projectId }
  );
  ...
  const remaining = Math.max((totalPlanPrice || 0) - paidTotal, 0);
    ...
    <div className="card dark:bg-surface-800 dark:border-surface-700">
      ...
      {isLoading ? (
        <div className="animate-pulse space-y-2">
          {[1, 2].map((i) => (
            <div key={i} className="h-12 bg-gray-50 dark:bg-surface-700 rounded-lg" />
          ))}
        </div>
      ) : payments.length === 0 ? (
        ...
        <div className="space-y-2">
          {payments.map((p) => {
            const phase = p.paymentPhase || 'advance';
            return (
              <div
                key={p._id}
                className="flex items-center justify-between gap-3 p-3 bg-gray-50 dark:bg-surface-700 rounded-xl"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <span
                    className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-full whitespace-nowrap ${PHASE_BADGE[phase] || PHASE_BADGE.advance}`}
                  >
                  ...
                  <div className="min-w-0">
                    ...
                    {p.invoiceNumber && (
                      <p className="text-[11px] text-gray-400 truncate">{p.invoiceNumber}</p>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-3 flex-shrink-0">
                  <span className={`${getPaymentStatusBadge(p.status)} capitalize`}>
                    {p.status}
                  </span>
